  ...
  try {
    ...
    for (let i = 0; i < faltam.length; i += 5) {
      ...
      if (todosItens.length > 0) {
        const { error } = await supabase.from('pedidos_itens').upsert((todosItens).map((__r:any) => ({...__r, empresa: 'bc'})), {
    ...
    await supabase.from('sync_log').insert({ tabela: 'pedidos_itens', tipo: 'sync_bc',
      tipo: 'itens',
