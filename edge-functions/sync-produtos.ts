...
Deno.serve(async () => {
  ...
  try {
    ...
    await supabase.from('sync_log').insert({ tabela: 'produtos', registros: total, status: 'ok', detalhes: duracao + 's' })
    ...
    await supabase.from('sync_log').insert({ tabela: 'produtos', registros: 0, status: 'error', erro: e.message })
