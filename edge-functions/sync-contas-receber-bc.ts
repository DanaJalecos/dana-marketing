  ...
  try {
    ...
    while (pageStart <= 20) {
      ...
      if (todasLinhas.length > 0) {
        await supabase.from('contas_receber').upsert((todasLinhas).map((__r:any) => ({...__r, empresa: 'bc'})), { onConflict: 'id' })
    ...
    await supabase.from('sync_log').insert({ tabela: 'contas_receber', tipo: 'sync_bc', registros: total, status: 'ok',
    ...
    await supabase.from('sync_log').insert({ tabela: 'contas_receber', tipo: 'sync_bc', registros: 0, status: 'error',
