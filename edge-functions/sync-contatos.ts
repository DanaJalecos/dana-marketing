...
Deno.serve(async () => {
  ...
  try {
    ...
    for (let page = 1; page <= 10; page++) {
      ...
      await supabase.from('contatos').upsert(rows, { onConflict: 'id' })
      total += rows.length
      await new Promise(r => setTimeout(r, 300))
    }

    const duracao = Math.round((Date.now() - inicio) / 1000)
    await supabase.from('sync_log').insert({ tabela: 'contatos', registros: total, status: 'ok', detalhes: duracao + 's' })
    ...
    await supabase.from('sync_log').insert({ tabela: 'contatos', registros: 0, status: 'error', erro: e.message })
