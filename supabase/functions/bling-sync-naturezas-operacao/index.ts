// ════════════════════════════════════════════════════════════════════════════
// bling-sync-naturezas-operacao — Lote 3 Tabela 1
// Cadastro paginado (volume ~30-50). Sem drill, sem filtro de data.
// Cron 1×/dia.
// ════════════════════════════════════════════════════════════════════════════
...
function mapNat(d: BlingNaturezaOp, loja_id: number) {
  return {
    id_bling: d.id,
    loja_id,
...
Deno.serve(async (_req) => {
  ...
  for (const { empresa, loja_id } of LOJAS) {
    const { data: logRow } = await sb.from('bling_sync_log').insert({
      tabela: 'bling_naturezas_operacao', loja_id, iniciado_em: new Date().toISOString(),
    ...
    try {
      ...
      let pagina = 1;
      while (true) {
        const r = await blingGet<{ data?: BlingNaturezaOp[] }>(
          token, '/naturezas-operacoes', { pagina, limite: 100 }
        );
        apiCalls++;
        if (!r.ok) throw new Error(`bling /naturezas-operacoes status=${r.status}: ${r.errorBody}`);
        const items = r.data?.data ?? [];
        if (!items.length) break;

        const rows = items.map(i => mapNat(i, loja_id));
        const { error } = await sb.from('bling_naturezas_operacao').upsert(rows, { onConflict: 'id_bling' });
