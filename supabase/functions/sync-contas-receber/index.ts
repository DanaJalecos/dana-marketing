// ════════════════════════════════════════════════════════════════════════════
// sync-contas-receber (MATRIZ, loja_id=203536978). Aceita ?situacao=1|2|3.
// ════════════════════════════════════════════════════════════════════════════
...
Deno.serve(async (req) => {
  ...
  try {
      ...
      for (const r of results) {
        ...
        for (const c of (r.data?.data ?? [])) {
          todasLinhas.push({
            ...
            conta_contabil: c.contaContabil || '',
          });
        }
      }
    ...
    await sb.from('sync_log').insert({
      tabela: 'contas_receber', registros: totalUpserted, status: 'ok',
    ...
    await sb.from('sync_log').insert({ tabela: 'contas_receber', registros: totalUpserted, status: 'error', erro: msg, detalhes: 'sit=' + situacao });
    if (logId) await sb.from('bling_sync_log').update({
