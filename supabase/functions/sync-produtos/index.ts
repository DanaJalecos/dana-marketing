// ════════════════════════════════════════════════════════════════════════════
// sync-produtos (MATRIZ, loja_id=203536978).
// ════════════════════════════════════════════════════════════════════════════
...
Deno.serve(async () => {
  ...
  try {
    ...
    await sb.from('sync_log').insert({ tabela: 'produtos', registros: totalUpserted, status: 'ok', detalhes: Math.round(dur / 1000) + 's' });
    ...
    await sb.from('sync_log').insert({ tabela: 'produtos', registros: totalUpserted, status: 'error', erro: msg });
    if (logId) await sb.from('bling_sync_log').update({
