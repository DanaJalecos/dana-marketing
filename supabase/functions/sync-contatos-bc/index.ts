// ════════════════════════════════════════════════════════════════════════════
// sync-contatos-bc (BC, loja_id=203550865).
// ════════════════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    for (let page = 1; page <= 10; page++) {
      ...
      const rows = items.map(c => ({
        ...
        telefone: c.telefone || '', celular: c.celular || '', empresa: EMPRESA,
      }));
    ...
    await sb.from('sync_log').insert({ tabela: 'contatos', tipo: 'sync_bc', registros: totalUpserted, status: 'ok', detalhes: Math.round(dur / 1000) + 's' });
    ...
    await sb.from('sync_log').insert({ tabela: 'contatos', tipo: 'sync_bc', registros: totalUpserted, status: 'error', erro: msg });
    if (logId) await sb.from('bling_sync_log').update({
