...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: 'minha_carteira',
      description: 'Retorna clientes vinculados a um VENDEDOR (via vendedor_mapping do Bling ou cliente_vendedor_manual). Padrão: vendedor logado. Admin/gerente pode passar parâmetro "vendedor" pra consultar carteira de OUTRO vendedor (ex: "5 clientes da Thamyres pra fechar esse mês", "top da Manuela", "carteira da Fabiana"). Use SEMPRE quando a pergunta for sobre "meus clientes", "minha carteira", "5 clientes da X", "top cliente da Y", "qual o melhor cliente da Z".',
      parameters: {
        type: 'object',
        properties: {
          empresa: { type: 'string', enum: ['matriz', 'bc', 'ambas'], description: 'Padrão: ambas' },
          vendedor: { type: 'string', description: 'Nome (parcial) do vendedor. APENAS admin/gerente pode usar isso pra consultar carteira de outra pessoa. Se omitido, usa o user logado. Ex: "Thamyres", "Manuela", "Fabiana".' },
  ...
  },
  {
    type: 'function',
    function: {
      name: 'clientes_sem_vendedor',
      description: 'Lista clientes da MATRIZ que estão SEM NENHUM VENDEDOR (pool aberto). Use quando o vendedor pedir opinião tipo "por qual cliente começo", "tem cliente bom sem dono?", "me sugere um cliente sem vendedor pra pegar", "qual o melhor cliente disponível", "o que vale a pena do pool". Regra de ouro: quem trabalhar (mudar status / registrar contato) esse cliente, ele vira automaticamente da carteira dessa pessoa. ESTA FERRAMENTA SEMPRE REFLETE O ESTADO ATUAL — cliente que já virou de algum vendedor NÃO aparece aqui. NUNCA recomende como "disponível/sem dono" um cliente que não veio nesta lista.',
      parameters: {
        type: 'object',
        properties: {
          ordenar_por: { type: 'string', enum: ['score', 'total_gasto', 'ultima_compra', 'dias_sem_compra', 'total_pedidos'], description: 'Padrão: score (decrescente) = melhores oportunidades primeiro. "total_gasto" pra maior valor histórico. "dias_sem_compra" crescente pra quem comprou recente.' },
          segmento: { type: 'string', description: 'Opcional. Valores: VIP, Frequente, Ocasional, Em Risco, Inativo, Novo' },
          busca: { type: 'string', description: 'Opcional: filtra por parte do nome do cliente' },
          limite: { type: 'number', description: 'Padrão 8, máx 30' }
        },
        required: []
      }
    }
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Lista criativos (artes, vídeos, banners) com filtros. Use pra "artes aguardando aprovação", "criativos aprovados essa semana", "artes da campanha X", "minhas artes" (designer), "publicadas no Instagram esse mês". Retorna título, status, designer, formato, briefing, observações.',
      parameters: {
        type: 'object',
        properties: {
          status: { type: 'string', enum: ['aguardando', 'aprovado', 'reprovado', 'todo', 'publicado'], description: 'Filtra por status do workflow' },
          designer: { type: 'string', description: 'Nome parcial do designer (busca ilike)' },
          briefing: { type: 'string', description: 'Nome parcial da campanha/briefing vinculado' },
          formato: { type: 'string', enum: ['reels', 'feed', 'stories', 'carrossel', 'banner', 'outro'], description: 'Formato do criativo' },
          dias_recentes: { type: 'number', description: 'Olhar últimos N dias (padrão 30, máx 180)' },
          minhas: { type: 'boolean', description: 'Se true e usuário for designer, filtra só do user logado' },
          limite: { type: 'number', description: 'Padrão 10, máx 50' }
        },
        required: []
      }
  ...
  },
  {
    type: 'function',
    function: {
      name: 'resumo_criativos',
      description: 'Panorama dos criativos: quantos em cada status, top designer por aprovação, taxa de aprovação, criativos publicados no mês. Use pra "como tá o pipeline de criativos", "qual designer mais publica", "estatísticas de aprovação".',
      parameters: {
        type: 'object',
        properties: {
          dias_recentes: { type: 'number', description: 'Janela em dias (padrão 30, máx 365)' }
        },
        required: []
      }
    }
  },
  {
    ...
    function: {
      ...
      description: 'Eventos do calendário (campanhas, promoções, lançamentos, prazos, feriados, eventos livres) num período. Use pra "o que tem agendado essa semana", "próximos lançamentos", "promoções de maio", "eventos da próxima quinzena", "tem feriado essa semana?".',
      parameters: {
        type: 'object',
        properties: {
          periodo: { type: 'string', description: '"hoje" | "semana" | "mes_atual" | "proximos_30" | "ano_atual" | YYYY-MM (mês específico)' },
          tipo: { type: 'string', enum: ['campanha', 'campanha_c360', 'campanha_interna', 'promocao', 'lancamento', 'evento', 'deadline', 'feriado'], description: 'Filtrar por tipo' },
          campanha: { type: 'string', description: 'Nome parcial da campanha vinculada' },
          limite: { type: 'number', description: 'Padrão 20, máx 100' }
        },
        required: []
      }
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Briefings de campanha salvos no Construtor. Use pra "briefings recentes", "qual briefing a Manu criou semana passada", "todos os briefings da campanha Inverno". Retorna título, público, conceito, oferta, canais, gancho, CTA, autor, data.',
      parameters: {
        type: 'object',
        properties: {
          busca: { type: 'string', description: 'Termo de busca (em título, conceito, oferta)' },
          autor: { type: 'string', description: 'Nome parcial do autor' },
          dias_recentes: { type: 'number', description: 'Olhar últimos N dias (padrão 90)' },
          limite: { type: 'number', description: 'Padrão 10, máx 50' }
        },
        required: []
      }
  ...
  },
  {
    type: 'function',
    function: {
      name: 'listar_influenciadores',
      description: 'Influenciadores cadastrados com filtros: status (ativo/pausado/inativo), nicho, região, performance. Use pra "quem são meus influenciadores ativos", "top influenciador por receita", "influenciadores do sul", "quanto a Bianca trouxe de venda esse mês".',
      parameters: {
        type: 'object',
        properties: {
          status: { type: 'string', enum: ['ativo', 'pausado', 'inativo'], description: 'Filtra por status' },
          nicho: { type: 'string', description: 'Nicho parcial (busca ilike)' },
          regiao: { type: 'string', description: 'Região (Sul, Sudeste, etc)' },
          busca: { type: 'string', description: 'Nome ou instagram parcial' },
          ordenar_por: { type: 'string', enum: ['receita', 'vendas', 'seguidores', 'recente'], description: 'Padrão receita' },
          limite: { type: 'number', description: 'Padrão 10, máx 50' }
        },
        required: []
      }
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Produtos com problema de estoque. Use pra "produtos críticos", "o que tá acabando", "estoque parado há 5 meses pra fazer promoção", "quantos produtos baixos tenho na matriz". 3 tipos: criticos (estoque < 3 un), baixos (3-5 un), parados (sem venda há +150d).',
      parameters: {
        type: 'object',
        properties: {
          tipo: { type: 'string', enum: ['criticos', 'baixos', 'parados', 'todos'], description: 'Padrão "todos" (resumo de cada categoria)' },
          empresa: { type: 'string', enum: ['matriz', 'bc', 'todas'], description: 'Padrão todas' },
          limite: { type: 'number', description: 'Padrão 15, máx 100' }
        },
        required: []
      }
    }
  },
  {
    ...
    function: {
      ...
      description: 'O que o usuário logado tem pra fazer AGORA. Junta: tarefas atribuídas pendentes, alertas pessoais não lidos do sininho, criativos aguardando aprovação (se for gerente). Use pra "o que eu tenho pra fazer", "minhas pendências", "o que tá esperando por mim hoje", "pendências importantes".',
      parameters: {
        type: 'object',
        properties: {
          incluir_concluidas: { type: 'boolean', description: 'Default false' }
        },
        required: []
      }
    }
...
const TOOL_SECOES: Record<string, string[]> = {
  ...
  // Criativos / Briefings / Influenciadores / Calendário
  listar_criativos:               ['criativos'],
  resumo_criativos:               ['criativos'],
  listar_eventos_calendario:      ['calendario', 'tarefas'],
  listar_briefings:               ['briefingvisual', 'construtor'],
  listar_influenciadores:         ['influenciadores'],
  // Estoque (admin/gerente_financeiro pela seção 'estoque' — quem não tem fica com erro_permissao)
  alertas_estoque:                ['estoque', 'home'],
  // Pendências pessoais — qualquer user logado pode chamar (filtra pelo user_id automaticamente)
  minhas_pendencias:              [],
  // Internas
...
async function executarFerramenta(nome: string, args: any, contextoUsuario: { cargo: string, secoes: Set<string>, profileId?: string, nome?: string }): Promise<any> {
  try {
    ...
    if (nome === 'minha_carteira') {
      const cargo = contextoUsuario.cargo
      const ehAdmin = cargo === 'admin' || cargo === 'gerente_comercial' || cargo === 'gerente_marketing'
      let profileIdAlvo = contextoUsuario.profileId
      let nomeAlvo = contextoUsuario.nome || 'voce'

      // Se passou "vendedor" e o user é admin/gerente, busca o profile do nome
      if (args.vendedor && ehAdmin) {
        const { data: profiles } = await supabaseAdmin.from('profiles')
          .select('id, nome')
          .ilike('nome', '%' + args.vendedor + '%')
          .limit(5)
        if (!profiles || profiles.length === 0) {
          return { erro: `Vendedor "${args.vendedor}" não encontrado. Vendedores cadastrados: cheque /Admin/Usuários.` }
        }
        if (profiles.length > 1) {
          return {
            erro: `Múltiplos vendedores batem com "${args.vendedor}": ${profiles.map(p => p.nome).join(', ')}. Seja mais específico.`,
            candidatos: profiles.map(p => p.nome)
          }
        }
        profileIdAlvo = profiles[0].id
        nomeAlvo = profiles[0].nome
      } else if (args.vendedor && !ehAdmin) {
        return { erro: 'Você não tem permissão pra consultar carteira de outro vendedor. Pergunte ao admin ou gerente.' }
      }

      if (!profileIdAlvo) return { erro: 'Usuário sem profile identificado' }
      const empresa = args.empresa || 'ambas'
      let q = supabaseAdmin.from('cliente_scoring_vendedor')
        ...
        .eq('vendedor_profile_id', profileIdAlvo)
      ...
      return {
        vendedor: nomeAlvo,
        consultado_por: contextoUsuario.nome || 'sistema',
    ...
    if (nome === 'clientes_sem_vendedor') {
      // Pool aberto da MATRIZ: vendedor_profile_id IS NULL = sem dono.
      // A view reflete o estado atual → cliente que já foi assumido
      // por alguém NÃO volta aqui (resolve "não falar mais dele").
      const ord = args.ordenar_por || 'score'
      const asc = (ord === 'dias_sem_compra' || ord === 'ultima_compra') ? true : false
      let q = supabaseAdmin.from('cliente_scoring_vendedor')
        .select('contato_nome, segmento, score, total_pedidos, total_gasto, ticket_medio, ultima_compra, dias_sem_compra')
        .eq('empresa', 'matriz')
        .is('vendedor_profile_id', null)
      if (args.segmento) q = q.eq('segmento', args.segmento)
      if (args.busca) q = q.ilike('contato_nome', '%' + String(args.busca).replace(/%/g, '') + '%')
      q = q.order(ord, { ascending: asc, nullsFirst: false }).limit(Math.min(+args.limite || 8, 30))
      const { data, error } = await q
      if (error) return { erro: error.message }
      return {
        contexto: 'Pool de clientes da Matriz SEM vendedor. Quem trabalhar o cliente (mudar status / registrar contato) vira dono automaticamente — primeiro que pega, leva. Lista já reflete o estado atual: clientes que viraram de alguém não aparecem.',
        ordenado_por: ord,
        filtros: { segmento: args.segmento || 'todos', busca: args.busca || null },
    ...
    if (nome === 'listar_criativos') {
      let q = supabaseAdmin.from('criativos').select('id, titulo, status, designer_nome, formato, briefing_titulo, observacoes, feedback, aprovado_em, feedback_em, publicado_em, plataforma, link_post, created_at, prazo_entrega, prioridade, solicitado_por_nome')
      if (args.status) q = q.eq('status', args.status)
      if (args.designer) q = q.ilike('designer_nome', '%' + args.designer + '%')
      if (args.briefing) q = q.ilike('briefing_titulo', '%' + args.briefing + '%')
      if (args.formato) q = q.eq('formato', args.formato)
      if (args.minhas && contextoUsuario.nome) q = q.ilike('designer_nome', '%' + contextoUsuario.nome.split(' ')[0] + '%')
      const dias = Math.min(args.dias_recentes || 30, 180)
      q = q.gte('created_at', new Date(Date.now() - dias * 86400000).toISOString())
      const limite = Math.min(args.limite || 10, 50)
      q = q.order('created_at', { ascending: false }).limit(limite)
      const { data, error } = await q
      if (error) return { erro: error.message }
      return {
        total: data?.length || 0,
        ...
        criativos: (data || []).map(c => ({
          titulo: c.titulo,
          status: c.status,
          designer: c.designer_nome,
          formato: c.formato,
          briefing: c.briefing_titulo,
          prioridade: c.prioridade,
          prazo_entrega: c.prazo_entrega,
          solicitado_por: c.solicitado_por_nome,
          observacoes: c.observacoes,
          feedback: c.feedback,
          aprovado_em: c.aprovado_em,
          feedback_em: c.feedback_em,
    ...
    if (nome === 'resumo_criativos') {
      const dias = Math.min(args.dias_recentes || 30, 365)
      const desde = new Date(Date.now() - dias * 86400000).toISOString()
      const { data, error } = await supabaseAdmin.from('criativos')
        .select('id, status, designer_nome, plataforma, aprovado_em, publicado_em, feedback_em, created_at')
        .gte('created_at', desde)
        .limit(1000)
      if (error) return { erro: error.message }
      const lista = data || []
      const porStatus: Record<string, number> = {}
      const porDesigner: Record<string, { total: number, aprovados: number, publicados: number }> = {}
      let aprovados = 0, reprovados = 0, publicados = 0
      lista.forEach(c => {
        porStatus[c.status] = (porStatus[c.status] || 0) + 1
        const d = c.designer_nome || '(sem designer)'
        if (!porDesigner[d]) porDesigner[d] = { total: 0, aprovados: 0, publicados: 0 }
        porDesigner[d].total++
        if (c.status === 'aprovado') { aprovados++; porDesigner[d].aprovados++ }
        if (c.status === 'reprovado') reprovados++
        if (c.status === 'publicado') { publicados++; porDesigner[d].publicados++ }
      })
      const totalDecididos = aprovados + reprovados
      const taxaAprovacao = totalDecididos > 0 ? Math.round(aprovados / totalDecididos * 100) : null
      const topDesigners = Object.entries(porDesigner)
        .sort((a, b) => b[1].publicados - a[1].publicados || b[1].aprovados - a[1].aprovados)
        .slice(0, 5)
        .map(([nome, stats]) => ({ designer: nome, ...stats }))
      return {
        periodo_dias: dias,
        total: lista.length,
    ...
    if (nome === 'listar_eventos_calendario') {
      const hoje = new Date()
      const periodo = args.periodo || 'mes_atual'
      let inicio: Date, fim: Date
      if (periodo === 'hoje') {
        inicio = new Date(hoje); inicio.setHours(0,0,0,0)
        fim = new Date(hoje); fim.setHours(23,59,59,999)
      } else if (periodo === 'semana') {
        inicio = new Date(hoje); fim = new Date(hoje); fim.setDate(fim.getDate() + 7)
      } else if (periodo === 'proximos_30') {
        inicio = new Date(hoje); fim = new Date(hoje); fim.setDate(fim.getDate() + 30)
      } else if (periodo === 'ano_atual') {
        inicio = new Date(hoje.getFullYear(), 0, 1); fim = new Date(hoje.getFullYear(), 11, 31)
      } else if (/^\d{4}-\d{2}$/.test(periodo)) {
        const [a, m] = periodo.split('-').map(Number)
        inicio = new Date(a, m - 1, 1); fim = new Date(a, m, 0)
      } else {
        // mes_atual (default)
      ...
      let q = supabaseAdmin.from('calendario')
        .select('id, titulo, tipo, data_inicio, data_fim, descricao, campanha_id, cor')
      ...
      q = q.order('data_inicio', { ascending: true }).limit(limite)
      const { data, error } = await q
      if (error) return { erro: error.message }
      let eventos = data || []
      // Filtro por campanha (em memória — campanhas_internas tem nome em outra tabela)
      if (args.campanha && eventos.length > 0) {
        const campIds = eventos.map(e => e.campanha_id).filter(Boolean)
        if (campIds.length > 0) {
          const { data: camps } = await supabaseAdmin.from('campanhas_internas')
            .select('id, titulo').in('id', campIds)
          const matchIds = new Set((camps || []).filter(c => (c.titulo || '').toLowerCase().includes(args.campanha.toLowerCase())).map(c => c.id))
          eventos = eventos.filter(e => !e.campanha_id || matchIds.has(e.campanha_id))
        }
      }
      return {
        ...
        eventos: eventos.map(e => ({
          ...
          tem_campanha_vinculada: !!e.campanha_id,
        })),
      }
    }

    if (nome === 'listar_briefings') {
      let q = supabaseAdmin.from('briefings_campanha')
        .select('id, titulo, publico, problema, conceito, oferta, canais, orcamento, gancho, cta, headline, criado_por_nome, created_at')
      if (args.autor) q = q.ilike('criado_por_nome', '%' + args.autor + '%')
      const dias = Math.min(args.dias_recentes || 90, 365)
      q = q.gte('created_at', new Date(Date.now() - dias * 86400000).toISOString())
      if (args.busca) q = q.or(`titulo.ilike.%${args.busca}%,conceito.ilike.%${args.busca}%,oferta.ilike.%${args.busca}%`)
      const limite = Math.min(args.limite || 10, 50)
      q = q.order('created_at', { ascending: false }).limit(limite)
      const { data, error } = await q
      if (error) return { erro: error.message }
      return {
        ...
        briefings: (data || []).map(b => ({
          ...
          cta: b.cta,
          headline: b.headline,
          autor: b.criado_por_nome,
          criado_em: b.created_at,
        })),
      }
    }

    if (nome === 'listar_influenciadores') {
      let q = supabaseAdmin.from('influenciadores').select('*')
      if (args.status) q = q.eq('status', args.status)
      if (args.nicho) q = q.ilike('nicho', '%' + args.nicho + '%')
      if (args.regiao) q = q.ilike('regiao', '%' + args.regiao + '%')
      if (args.busca) q = q.or(`nome.ilike.%${args.busca}%,instagram.ilike.%${args.busca}%`)
      const ord = args.ordenar_por || 'receita'
      const col = ord === 'vendas' ? 'vendas_geradas' : ord === 'seguidores' ? 'seguidores' : ord === 'recente' ? 'inicio_parceria' : 'receita'
      ...
      return {
        ...
        influenciadores: (data || []).map(i => ({
          ...
          usos_cupom: i.usos_cupom,
          vendas_geradas: i.vendas_geradas,
          receita: i.receita,
          inicio_parceria: i.inicio_parceria,
        })),
      }
    }

    if (nome === 'alertas_estoque') {
      const tipo = args.tipo || 'todos'
      const empresa = args.empresa || 'todas'
      const limite = Math.min(args.limite || 15, 100)
      const out: any = {}

      // Resumo via RPC se for todos, ou puxa lista específica
      try {
        const { data: kpiData } = await supabaseAdmin.rpc('estoque_kpis', { empresa_filter: empresa })
        const kpi = kpiData?.estoque_kpis || kpiData || {}
        out.resumo = kpi
      } catch (e: any) { out.resumo_erro = e?.message }

      if (tipo === 'criticos' || tipo === 'baixos' || tipo === 'todos') {
        try {
          const { data } = await supabaseAdmin.rpc('listar_estoque_baixo', { empresa_filter: empresa })
          let lista = data || []
          if (tipo === 'criticos') lista = lista.filter((p: any) => Number(p.estoque_virtual) < 3)
          else if (tipo === 'baixos') lista = lista.filter((p: any) => Number(p.estoque_virtual) >= 3 && Number(p.estoque_virtual) <= 5)
          out.ativos = lista.slice(0, limite).map((p: any) => ({
            nome: p.nome, codigo: p.codigo, empresa: p.empresa,
            estoque_atual: p.estoque_virtual,
            vendas_90d: p.qtd_vendida_90d,
            media_mes: p.media_mes,
            produzir_sugerido: p.qtd_sugerida_producao,
          }))
        } catch (e: any) { out.ativos_erro = e?.message }
      }

      if (tipo === 'parados' || tipo === 'todos') {
        try {
          const { data } = await supabaseAdmin.rpc('listar_estoque_parados', { empresa_filter: empresa })
          out.parados = (data || []).slice(0, limite).map((p: any) => ({
            nome: p.nome, codigo: p.codigo, empresa: p.empresa,
            estoque: p.estoque_virtual,
            valor_parado: p.valor_parado,
            ultima_venda: p.ultima_venda,
            dias_sem_vender: p.dias_sem_vender,
          }))
        } catch (e: any) { out.parados_erro = e?.message }
      }
      return out
    }

    if (nome === 'minhas_pendencias') {
      const uid = contextoUsuario.profileId
      const userNome = contextoUsuario.nome || ''
      if (!uid) return { erro: 'usuário não identificado' }
      const out: any = { resumo: {} }

      // 1. Tarefas atribuídas (match por nome do profile — tarefas.responsavel é TEXT)
      try {
        let q = supabaseAdmin.from('tarefas')
          .select('id, titulo, prioridade, prazo, coluna, descricao, tag, concluido')
          .or(`responsavel.ilike.%${userNome}%,responsavel.eq.${userNome}`)
        if (!args.incluir_concluidas) q = q.eq('concluido', false)
        q = q.order('prazo', { ascending: true, nullsFirst: false }).limit(30)
        const { data: tarefas } = await q
        out.tarefas_pendentes = (tarefas || []).filter(t => t.coluna !== 'arquivo').map(t => ({
          titulo: t.titulo, prioridade: t.prioridade, prazo: t.prazo,
          coluna: t.coluna, tag: t.tag,
        }))
        out.resumo.tarefas = out.tarefas_pendentes.length
      } catch (e: any) { out.tarefas_erro = e?.message }

      // 2. Alertas pessoais não lidos
      try {
        const { data: alertas } = await supabaseAdmin.from('alertas')
          .select('tipo, titulo, mensagem, nivel, link_ref, created_at')
          .eq('destinatario_id', uid)
          .eq('lido', false)
          .neq('tipo', 'estoque_baixo')
          .order('created_at', { ascending: false })
          .limit(20)
        out.alertas_nao_lidos = (alertas || []).map(a => ({
          titulo: a.titulo, mensagem: a.mensagem, nivel: a.nivel,
          tipo: a.tipo, criado_em: a.created_at,
        }))
        out.resumo.alertas = out.alertas_nao_lidos.length
      } catch (e: any) { out.alertas_erro = e?.message }

      // 3. Criativos aguardando aprovação (só pra gerente/admin)
      const cargo = contextoUsuario.cargo
      if (cargo === 'admin' || cargo === 'gerente_marketing' || cargo === 'gerente_comercial') {
        try {
          const { data: crs } = await supabaseAdmin.from('criativos')
            .select('id, titulo, designer_nome, formato, created_at')
            .eq('status', 'aguardando')
            .order('created_at', { ascending: true })
            .limit(20)
          out.criativos_pra_aprovar = (crs || []).map(c => ({
            titulo: c.titulo, designer: c.designer_nome, formato: c.formato, criado_em: c.created_at,
          }))
          out.resumo.criativos_aprovar = out.criativos_pra_aprovar.length
        } catch (e: any) { out.criativos_erro = e?.message }
      }

      out.resumo.total = (out.resumo.tarefas || 0) + (out.resumo.alertas || 0) + (out.resumo.criativos_aprovar || 0)
      return out
    }
...
----------------------------------------------------------
REGRA CRITICA: NUNCA use buscar_contato pra termos analiticos
----------------------------------------------------------

buscar_contato é APENAS pra busca por NOME de cliente. Se o usuário disse:
- "melhor score", "maior", "menor", "pior", "top X", "mais gastou"
- "potencial", "vai fechar", "pra bater meta", "mais provável de comprar"
- "VIP", "em risco", "inativo", "sumiu", "novo"
- "quem deve mais", "quem está parado", "quem comprou X"

NÃO é busca de nome. É QUERY ANALÍTICA. Use:
- minha_carteira (se for vendedor)
- top_clientes (se for admin/gerente)
- alertas_cliente360 (se for análise de oportunidade)
- listar_segmentos_c360 (se for segmentação)

EXEMPLOS REAIS DE ERRO QUE NÃO PODE REPETIR:
❌ User: "qual minha cliente com o melhor score"
❌ Errado: buscar_contato(nome="melhor score") → "não encontrei ninguém"
✅ Certo: minha_carteira(ordenar_por="score", limite=1) → retorna o top

❌ User: "quem é meu cliente que mais gasta"
❌ Errado: buscar_contato(nome="mais gasta")
✅ Certo: minha_carteira(ordenar_por="total_gasto", limite=1)

❌ User: "5 clientes pra bater meta esse mês"
✅ Certo: minha_carteira(ordenar_por="score", limite=5) — pega quem tem maior probabilidade de conversão
...
- top_clientes, top_produtos: rankings (admin/gerente — NUNCA pra vendedor)
...
- buscar_contato: ⚠️ APENAS busca por NOME ESPECÍFICO. Ex: "quem é a Maria Silva?", "telefone da clínica X". Nunca pra termos vagos/analíticos.
- info_produto: estoque, preço, busca por código/nome
- minha_carteira: FERRAMENTA PRINCIPAL pra perguntas sobre carteira de UM vendedor. Padrão usa o user logado. Admin/gerente pode passar parâmetro "vendedor" pra consultar OUTRO vendedor (ex: "5 clientes da Thamyres pra bater meta" → minha_carteira({vendedor:"Thamyres", ordenar_por:"score", limite:5})). Ordena por score/gasto/dias_sem_compra. Aceita filtro segmento.
- clientes_sem_vendedor: pool de clientes da MATRIZ SEM dono. Use quando o vendedor pedir conselho de "por qual cliente começo", "tem cliente bom sem vendedor?", "me indica um pra eu pegar", "o que vale a pena do pool". Recomende os de maior score/valor e explique que basta ele trabalhar o cliente (mudar status / registrar contato) que vira automaticamente da carteira dele — primeiro que pega, leva.
- listar_campanhas_internas: campanhas de equipe (organização interna), filtro "minhas=true" pega só do usuário.
- detalhe_campanha_interna: tudo sobre uma campanha (equipe, expedições, comentários, materiais, timeline).
- listar_criativos: artes/criativos com filtro de status (aguardando/aprovado/reprovado/publicado), designer, formato, briefing. Use pra "artes pendentes de aprovação", "minhas artes aprovadas", "criativos da campanha X", "que designer publicou mais esse mês".
- resumo_criativos: panorama (quantos em cada status, top designer, taxa aprovação)
- listar_eventos_calendario: eventos do calendário (campanhas, promoções, lançamentos, prazos, feriados) por período. Use pra "o que tem agendado essa semana", "próximos eventos", "lançamentos do mês".
- listar_briefings: briefings de campanha salvos. Use pra "briefings recentes", "quais briefings a Manu criou", "briefing da campanha X".
- listar_influenciadores: influenciadores cadastrados com status, nicho, região, receita gerada. Use pra "quem são meus influenciadores ativos", "top influenciador por receita", "influenciadores de SP".
- alertas_estoque: produtos com estoque baixo/crítico ou parados há +5 meses. Use pra "produtos críticos", "o que tá acabando", "produtos parados pra fazer promoção".
- minhas_pendencias: o que esse user logado tem pra fazer AGORA — tarefas atribuídas + alertas pessoais + criativos pra aprovar (se for gerente). Use pra "o que eu tenho pra fazer", "minhas pendências", "que ações estão esperando por mim".

IMPORTANTE: se o usuário for cargo=vendedor, ele SÓ tem acesso à própria carteira. Quando ele perguntar genericamente sobre "clientes" (ex: "qual cliente tem melhor score", "meus VIPs", "quem sumiu"), assuma que se refere à carteira dele e use minha_carteira. Não use resumo_cliente360, listar_segmentos_c360 ou buscar_contato pra perguntas de vendedor — essas retornam dados globais que ele não deve ver. EXCEÇÃO: se ele pedir sugestão de cliente NOVO pra prospectar / "por onde começar" / cliente sem dono → use clientes_sem_vendedor (pool aberto da Matriz, todos os vendedores podem pegar).

REGRA DO POOL SEM VENDEDOR (crítica): só fale de clientes sem dono usando a ferramenta clientes_sem_vendedor, e SEMPRE no momento da pergunta (ela já reflete o estado atual). Se um cliente NÃO veio no resultado dessa ferramenta, ele JÁ TEM dono — então NUNCA o sugira como "disponível", "sem vendedor" ou "pra pegar", mesmo que tenha sido citado antes nessa conversa. Não guarde/reuse listas antigas do pool: cliente que virou de um vendedor sai do pool e não pode mais ser oferecido a ninguém. Nunca invente nomes.
...
- consultar_tabela: SELECT genérico em qualquer tabela permitida, com filtros (só admin)

ESTRATÉGIA DE DECISÃO:
1. Pergunta sobre PESSOA ESPECÍFICA com nome conhecido ("Maria Silva", "Clínica Souza") → buscar_contato
2. Pergunta com TERMO ANALÍTICO ("melhor", "top", "VIP", "potencial", "sumiu") → minha_carteira (vendedor) ou top_clientes (admin)
3. Pergunta sobre tarefas/kanban → buscar_tarefas ou resumo_kanban
4. Pergunta sobre criativos/artes → listar_criativos ou resumo_criativos
5. Pergunta sobre eventos/calendário → listar_eventos_calendario
6. Pergunta sobre influenciadores → listar_influenciadores
7. Pergunta sobre estoque → alertas_estoque ou info_produto
8. "O que tenho pra fazer hoje" → minhas_pendencias
9. "Por qual cliente começo / cliente sem dono / me sugere um pra pegar / o que tem no pool" → clientes_sem_vendedor
10. Pergunta nova/desconhecida → listar_schema + consultar_tabela
