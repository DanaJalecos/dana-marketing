...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3.1 Escopo vendedor: carteira própria OU cliente sem dono (pool).
    // Pool = vendedor_profile_id NULL → qualquer vendedor pode analisar
    // antes de assumir (insight é read-only, não atribui o cliente).
    if (cargo === 'vendedor' || cargo === 'vendedora') {
      const row = await supaSingle(
        `cliente_scoring_vendedor?empresa=eq.${empresa}&contato_nome=eq.${enc(contato_nome)}&select=vendedor_profile_id&limit=1`
      )
      const dono = row?.vendedor_profile_id || null
      const liberado = !dono || dono === user.id   // sem dono OU é meu
      if (!liberado) {
        return json({
          error: 'Este cliente já tem outro vendedor. Você só gera insight dos seus clientes ou dos que estão sem vendedor (pool).',
        }, 403)
      }
    }
